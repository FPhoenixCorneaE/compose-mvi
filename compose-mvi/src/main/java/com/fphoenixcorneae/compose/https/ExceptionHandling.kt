...
object ExceptionHandling {
    ...
    private const val PARSE_ERROR = "数据解析错误"

    /** 证书验证失败 */
    private const val SSL_ERROR_CODE = 1004
    private const val SSL_ERROR = "证书验证失败"

    fun deal(e: Throwable?): Throwable = when (e) {
        is HttpException -> {
            ApiException(code = e.code(), msg = NETWORK_ERROR, cause = e)
        }
        ...
        is ConnectException -> {
            ApiException(code = NETWORK_ERROR_CODE, msg = NETWORK_ERROR, cause = e)
        }
        is javax.net.ssl.SSLException -> {
            ApiException(code = SSL_ERROR_CODE, msg = SSL_ERROR, cause = e)
        }
        is ConnectTimeoutException, is java.net.SocketTimeoutException, is java.net.UnknownHostException -> {
            ApiException(
        ...
        is ServerException -> {
            ...
            ApiException(code = UNKNOWN_ERROR_CODE, msg = UNKNOWN_ERROR, cause = e)
        }
    }
}
